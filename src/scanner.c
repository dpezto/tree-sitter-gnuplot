...
unsigned tree_sitter_gnuplot_external_scanner_serialize(void *payload,
  ...
  // return word_length + 2; // +1 for ending_char, +1 for null terminator
}
...
void tree_sitter_gnuplot_external_scanner_deserialize(void *payload,
  ...
  // }
  //
  // // Deserialize the ending_char first
...
//       consume(lexer); // Consume the escaped ending_char
...
//     while (lexer->lookahead != 0 && !iswspace(lexer->lookahead) && lexer->lookahead != ending_char) {
...
//   while (lexer->lookahead != 0 && lexer->lookahead != ending_char) {
//     if (lexer->lookahead == '\\') {
