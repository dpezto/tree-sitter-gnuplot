# tree-sitter-gnuplot

[![PRs Welcome][prs-badge]](https://makeapullrequest.com)

[Gnuplot](http://gnuplot.info) grammar for tree-sitter.

- Gnuplot 6.x

Everything should be supported but experimental and deprecated features.

## References

- [Gnuplot 6.0 documentation](http://gnuplot.info/docs_6.0/Gnuplot_6.pdf)

[prs-badge]: https://img.shields.io/badge/PRs-welcome-brightgreen.svg?style=flat&logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%%2BIDwvZz48L3N2Zz4%3D
